...
module physderivs_mod
  ...
  use common_mod, only : iotempflag, iotemp_un,bottom_flux_un, &
                       bbcnsv, flag_bbcnsv
...
contains
  ...
  subroutine physderivs(istep,bio_prev,dydt,bioparams)
    ...
    ! 计算底部扩散通量
    double precision :: bottom_diff_flux(NumStateVar)
    double precision :: bottom_adv_flux(NumStateVar)
    double precision :: bbc_conc
    integer :: isv_bottom
    double precision :: dz_bottom
	...
	do ii = 1,numstatevar
        ...
        ! --- 新增：底部边界通量计算和输出
        dz_bottom = zmid(nz+1) - zmid(nz)
        
        do isv_bottom = 1, NumStateVar
            ! 获取底部边界浓度（与physderivs_init中相同的逻辑）
            if (bbcnsv(isv_bottom,istep) .lt. -1.0d0) then
                bbc_conc = bio_prev(nz,isv_bottom) - &
                          (bio_prev(nz-1,isv_bottom) - bio_prev(nz,isv_bottom))
            elseif (flag_bbcnsv(isv_bottom) .eq. 1.0d0) then
                bbc_conc = bbcnsv(isv_bottom,istep-1)
            elseif (flag_bbcnsv(isv_bottom) .ge. 2.0d0) then
                bbc_conc = bio_prev(nz,isv_bottom) + bbcnsv(isv_bottom,istep-1)
            else
                bbc_conc = bio_prev(nz,isv_bottom)
            endif
            bbc_conc = max(bbc_conc, c0)
            
            ! 扩散通量（vertmix过程）
            bottom_diff_flux(isv_bottom) = -rkz(nz+1,istep) * &
                          (bio_prev(nz,isv_bottom) - bbc_conc) / dz_bottom
            
            ! 对流/沉降通量（vertadv和sink过程）
            if (wvel(nz+1,istep) > c0) then
                bottom_adv_flux(isv_bottom) = wvel(nz+1,istep) * bbc_conc
            else
                bottom_adv_flux(isv_bottom) = wvel(nz+1,istep) * &
                                              bio_prev(nz,isv_bottom)
            end if
        end do
