...
u_nh4 = max(0.0d0, min(0.3d0, u_nh4)) ! 数值约束在 [0, 1]
...
! 最终 SP 的 15N 吸收方程
    if (y(iNH4) < 1.0d-3) then
        growSP15nh4 = growSPnh4 * (rNH4 / (1.0d0 + rNH4))
    else
...
u_no3 = max(0.0d0, min (0.3d0, u_no3)) ! 数值约束在 [0, 1]
...
! 最终 SP 的 15N 吸收方程
if (y(iNO3) < 1.0d-3) then
   growSP15no3 = growSPno3 * (rNO3 / (1.0d0 + rNO3))
else
...
! 浮游植物同化DIN的总15N
 growSP15n   = growSP15nh4 + growSP15no3
    ...
    grazSPc = mu_PRT * y(iPRTc) * y(iSPc) * y(iSPc) &
     ...
     u_sp_graz = max(0.0d0, min(0.3d0, u_sp_graz)) ! 数值约束在 [0, 1]
...
factor_sp_graz = 1.0d0 
   ...
   growTR15nf = growTRnf * (beta_nf / (1.0d0 + beta_nf))
    ...
    !--------------------------------------------------------------------
     if (y(iNH4) < 1.0d-3) then
       growTR15nh4 = growTRnh4 * (rNH4 / (1.0d0 + rNH4))
    else
    ...
    !--------------------------------------------------------------------
    if (y(iNO3) < 1.0d-3) then
        growTR15no3 = growTRno3 * (rNO3 / (1.0d0 + rNO3))
    else
...
u_tr_excr = max(0.0d0, min(0.3d0, u_tr_excr))
...
factor_tr_excr = 1.0d0 
    ...
    !--------------------------------------------------------------------
     ...
     u_tr_graz = max(0.0d0, min(0.3d0, u_tr_graz))! 数值约束在 [0, 1]
    ...
    !--------------------------------------------------------------------
     if (y(iNH4) < 1.0d-3) then
         growDDA15nh4 = growDDAnh4 * (rNH4 / (1.0d0 + rNH4))
    else
    ...
    !--------------------------------------------------------------------
    if (y(iNO3) < 1.0d-3) then
         growDDA15no3 = growDDAno3 * (rNO3 / (1.0d0 + rNO3))
    else
    ...
    u_dda_excr = max(0.0d0, min(0.3d0, u_dda_excr))
   ...
   ! DDA grazed
    ...
    u_dda_graz = max(0.0d0, min(0.3d0, u_dda_graz)) ! 数值约束在 [0, 1]
   ...
   ! 15N 吸收与固氮
    ...
    !--------------------------------------------------------------------
    if (y(iNH4) < 1.0d-3) then
         growUN15nh4 = growUNnh4 * (rNH4 / (1.0d0 + rNH4))
    else
    ...
    !--------------------------------------------------------------------
    if (y(iNO3) < 1.0d-3) then
         growUN15no3 = growUN15no3* (rNO3 / (1.0d0 + rNO3))
    else
    ...
    u_un_excr = max(0.0d0, min(0.3d0, u_un_excr))
   ...
   ! UN grazed
    ...
    u_un_graz = max(0.0d0, min(0.3d0, u_un_graz)) ! 数值约束在 [0, 1]
    ...
    if (y(iNH4) < 1.0d-3) then
         growBA15nh4 = growBAnh4 * (rNH4 / (1.0d0 + rNH4))
    else
    ...
    !--------------------------------------------------------------------
    if (y(iNO3) < 1.0d-3) then
         growBA15no3 = growBA15no3* (rNO3 / (1.0d0 + rNO3))
    else
  ...
  ! BA再矿化的15N吸收方程
   remiBA15n = remiBAn * (beta_BA_remi/ (1.0d0 + beta_BA_remi))
    ...
    u_ba_graz = max(0.0d0, min(0.3d0, u_ba_graz)) ! 数值约束在 [0, 1]
...
remiPRT15n = remiPRTn * (beta_PRT_remi / (1.0d0 + beta_PRT_remi))
    ...
    grazPRTc = mu_MZ * y(iMZc) * y(iPRTc) * y(iPRTc) &
     ...
     u_prt_graz = max(0.0d0, min(0.3d0, u_prt_graz)) ! 数值约束在 [0, 1]
    ...
    !-----------------------------------------------------------------------
     ...
     !--------------------------------------------------------------------
     !  计算硝化过程的beta 值 
     !beta_nitrf = rNH4 * (1.0d0 - epsilon_nitrf / 1000.0d0)
     !nitrf15= nitrf * (beta_nitrf /(1.0d0+ beta_nitrf ))
     actual_eps_nitrf = epsilon_nitrf * (y(iNH4) / (y(iNH4) + 0.01d0)) 
     beta_nitrf = rNH4 * (1.0d0 - actual_eps_nitrf / 1000.0d0)
     nitrf15 = nitrf * (beta_nitrf / (1.0d0 + beta_nitrf + eps))
