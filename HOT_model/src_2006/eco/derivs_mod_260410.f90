...
u_nh4 = max(0.0d0, min(0.98d0, u_nh4)) ! 数值约束在 [0, 1]
...
u_no3 = max(0.0d0, min (0.98d0, u_no3)) ! 数值约束在 [0, 1]
...
! 浮游植物同化DIN的总15N
 growSP15n   = growSP15nh4 + growSP15no3
    ...
    grazSPc = mu_PRT * y(iPRTc) * y(iSPc) * y(iSPc) &
     ...
     u_sp_graz = max(0.0d0, min(0.98d0, u_sp_graz)) ! 数值约束在 [0, 1]
...
u_tr_excr = max(0.0d0, min(0.98d0, u_tr_excr))
...
factor_tr_excr = 1.0d0 
    ...
    !--------------------------------------------------------------------
     ...
     u_tr_graz = max(0.0d0, min(0.98d0, u_tr_graz))! 数值约束在 [0, 1]
...
u_dda_excr = max(0.0d0, min(0.98d0, u_dda_excr))
...
u_dda_graz = max(0.0d0, min(0.98d0, u_dda_graz)) ! 数值约束在 [0, 1]
...
u_un_excr = max(0.0d0, min(0.98d0, u_un_excr))
...
u_un_graz = max(0.0d0, min(0.98d0, u_un_graz)) ! 数值约束在 [0, 1]
...
remiBA15n = remiBAn * (rBA/(1.0d0+rBA))
...
u_ba_graz = max(0.0d0, min(0.98d0, u_ba_graz)) ! 数值约束在 [0, 1]
...
factor_ba_graz = 1.0d0 
   ...
   factor_ba_graz= -((1.0d0 - u_ba_graz) / u_ba_graz* log(1.0d0 - u_ba_graz))
    ...
    remiPRT15n = remiPRTn * (rPRT/(1.0d0+rPRT))
    ...
    grazPRTc = mu_MZ * y(iMZc) * y(iPRTc) * y(iPRTc) &
     ...
     u_prt_graz = max(0.0d0, min(0.98d0, u_prt_graz)) ! 数值约束在 [0, 1]
