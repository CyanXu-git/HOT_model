...
! r_SDOMrefr    = bioparams(ir_SDOMrefr     )
  ...
  !----------------------------------------------
   ...
   ! 添加15N同位素分馏效应
    ...
    u_nh4 = max(0.0d0, min(0.5d0, u_nh4)) ! 数值约束在 [0, 1]
    ...
    u_no3 = max(0.0d0, min (0.5d0, u_no3)) ! 数值约束在 [0, 1]
    ...
    grazSPc = mu_PRT * y(iPRTc) * y(iSPc) * y(iSPc) &
     ...
     u_sp_graz = max(0.0d0, min(0.5d0, u_sp_graz)) ! 数值约束在 [0, 1]
   ...
   END IF  
    ...
    u_tr_excr = max(0.0d0, min(0.5d0, u_tr_excr))
    ...
    u_tr_graz = max(0.0d0, min(0.5d0, u_tr_graz))! 数值约束在 [0, 1]
    ...
    u_dda_excr = max(0.0d0, min(0.5d0, u_dda_excr))
   ...
   ! DDA grazed
    ...
    u_dda_graz = max(0.0d0, min(0.5d0, u_dda_graz)) ! 数值约束在 [0, 1]
   ...
   ! 15N 吸收与固氮
    ...
    u_un_excr = max(0.0d0, min(0.5d0, u_un_excr))
   ...
   ! UN grazed
    ...
    u_un_graz = max(0.0d0, min(0.5d0, u_un_graz)) ! 数值约束在 [0, 1]
   ...
   remiBA15n = remiBAn * (beta_BA_remi/ (1.0d0 + beta_BA_remi))
    ...
    u_ba_graz = max(0.0d0, min(0.5d0, u_ba_graz)) ! 数值约束在 [0, 1]
    ...
    !--------------------------------------------------------------------
     ...
     u_prt_graz = max(0.0d0, min(0.5d0, u_prt_graz)) ! 数值约束在 [0, 1]
