...
!----------------------------------------------
    do iz = nz, 1, -1
       ...
       call DERIVS_B(bio_prev(iz,:), adbio_prev(iz,:), &  ! y, yb
            addydt_bio(iz,:), addydt_bio_b(iz,:), & 
            addydt_diag(iz,:), addydt_diag_b(iz,:), & ! dydtt_diag, dydtt_diagb
            istep, iz,bioparams, adbioparams) 
...
!where (nsed_pred(1:ndat_sed) .gt. c0)
    ...
    do it = nt, 1, -1
       ...
       do ised = 1, ndat_sed(xcstn15)
          ...
          if (t2trap .gt. btime_csed(xcstn15,ised) .and. &
               t2trap .le. etime_csed(xcstn15,ised)) then               
             adbio(ised_loc-1,iDET15n,it_temp) = adbio(ised_loc-1,iDET15n,it_temp)+ &
                  adcsed_pred(xcstn15,ised)*wnsv(iDET15n)*mwN* &
                  exp(-(z2trap*bioparams(iremin)*bioparams(iremin_prf_n)*&
                  (1.0d0-bioparams(iepsilon_remin)/1000.0d0)/wnsv(iDET15n)))
             adbioparams(iremin) = adbioparams(iremin)- &
                  adcsed_pred(xcstn15,ised)*wnsv(iDET15n)*mwN* &
                  bio(ised_loc-1,iDET15n,it_temp)*(z2trap* bioparams(iremin_prf_n)*&
                  (1.0d0-bioparams(iepsilon_remin)/1000.0d0) /wnsv(iDET15n))* &
                  exp(-(z2trap*bioparams(iremin)*bioparams(iremin_prf_n)*&
                  (1.0d0-bioparams(iepsilon_remin)/1000.0d0)/wnsv(iDET15n)))
             adbioparams(iremin_prf_n) = adbioparams(iremin_prf_n)- &
                  adcsed_pred(xcstn15,ised)*wnsv(iDET15n)*mwN* &
                  bio(ised_loc-1,iDET15n,it_temp)*(z2trap*bioparams(iremin)*&
                  (1.0d0-bioparams(iepsilon_remin)/1000.0d0)/wnsv(iDET15n))* &
                  exp(-(z2trap*bioparams(iremin)*bioparams(iremin_prf_n)*&
                  (1.0d0-bioparams(iepsilon_remin)/1000.0d0)/wnsv(iDET15n)))
             adbioparams(iepsilon_remin) = adbioparams(iepsilon_remin)+ & 
                  adcsed_pred(xcstn15,ised)*wnsv(iDET15n)*mwN* &
                  bio(ised_loc-1,iDET15n,it_temp)*(z2trap*bioparams(iremin)*&
                  bioparams(iremin_prf_n)/wnsv(iDET15n)/1000.0d0)* &
                  exp(-(z2trap*bioparams(iremin)*bioparams(iremin_prf_n)*&
                  (1.0d0-bioparams(iepsilon_remin)/1000.0d0)/wnsv(iDET15n)))
             adwnsv(iDET15n) = adwnsv(iDET15n) + adcsed_pred(xcstn15,ised) * &
                  mwN*bio(ised_loc-1,iDET15n,it_temp) * &
                  (exp(-(z2trap*bioparams(iremin)*bioparams(iremin_prf_n)*&
                  (1.0d0-bioparams(iepsilon_remin)/1000.0d0)/wnsv(iDET15n)))+ &
                  z2trap*bioparams(iremin)*bioparams(iremin_prf_n)*(1.0d0-&
                  bioparams(iepsilon_remin)/1000.0d0)/wnsv(iDET15n) * &
                  exp(-(z2trap*bioparams(iremin)*bioparams(iremin_prf_n)*&
                  (1.0d0-bioparams(iepsilon_remin)/1000.0d0)/wnsv(iDET15n))))
          endif
       end do
