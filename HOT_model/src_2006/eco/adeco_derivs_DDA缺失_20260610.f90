...
!==============================================
  ...
  !==============================================
    ...
    double precision, dimension(NumStateVar) :: addydt_bio_b 

    double precision, dimension(NumDiagVar) :: diag_fwd
    double precision, dimension(NumDiagVar) :: addydt_diag_b
...
!----------------------------------------------
	...
	call DERIVS_B(bio_prev(iz,:), adbio_prev(iz,:), &
                     addydt_bio(iz,:), addydt_bio_b, &
                     addydt_diag(iz,:), addydt_diag_b, &
                     istep, iz, bioparams, adbioparams,rlt, rltb)
       
       ! 回传梯度（关键：必须有这一步）
       adbio_prev(iz,:) = adbio_prev(iz,:) + addydt_bio_b(:)
    ...
    ! 1. 当前深度的正向光照
      ...
      ! rltb = 0.0d0
       ...
       ! 2. 重新计算当前 iz、istep 的正向 derivs 输出
!       dydt_fwd(:) = 0.0d0
!      diag_fwd(:) = 0.0d0
!
!      call derivs(bio_prev(iz,:), dydt_fwd, diag_fwd, &
!                   istep, iz, bioparams)
!       ! 3. 保存 dydt 和 diag 的伴随种子
!       dydt_b(:) = addydt_bio(iz,:)
!       diag_b(:) = addydt_diag(iz,:)

       ! 4. 伴随种子被 DERIVS_B 消耗后，应清零原数组，避免重复使用
!       addydt_bio(iz,:)  = 0.0d0
!       addydt_diag(iz,:) = 0.0d0

       ! 5. 调用 Tapenade 3.16 生成的 12 参数 DERIVS_B
!       call DERIVS_B(bio_prev(iz,:), adbio_prev(iz,:), &
!                     dydt_fwd, dydt_b, &
!                     diag_fwd, diag_b, &
!                     istep, iz, bioparams, adbioparams, rlt, rltb)

       ! 6. rlt = par(iz)，所以 rltb 必须回传给 adpar(iz)
...
! ***Required***
  subroutine adget_costpred(bio,diag,bioparams,adcpred,adcsed_pred,adbio, &
    ...
    use eco_params, only: iPO4,iBAc,iprBAc,iSDOMc,iSDOMn,iSDOMp,&
                          iSPc,iTRc,iUNc,iPRTc,iDETc,iSPn,iSPp, &
			      iTRn,iTRp,iUNn,iUNp,iPRTn,iPRTp,iDETn,&
			      iDET15n,iepsilon_remin,iDETp,iSPchl,iTRchl,iUNchl !Luo_MB
