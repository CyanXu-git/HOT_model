...
!-------------------------------------------------------------------------
  subroutine read_bio
    ...
    use eco_params, only : NumStateVar,iSPc,fname_bio_suffix,&
        ...
        iDETn, iNH4, iNO3, iSP15n, iTR15n, &
        iDDA15n, iUN15n, iBA15n, iPRT15n, &
        iMZ15n, iLDOM15n, iSDOM15n,&
        iDET15n, i15NH4, i15NO3
...
! ========================================================== 
! 【15N 初始值与底部边界条件补充说明】 
! 模型通过上面的循环，会自动读取您为 15N 新增加的.init 和 _bbc.dat 文件。
! 暂时还没准备好 15N 独立的数据文件，为了使模型能够顺利初始化并运行， 
!将基于常规 N 状态变量和自然同位素丰度 
! (如0.00366) 直接推算并覆盖 15N 的初始浓度与底部边界浓度。
!==========================================================
!bio0_glob(:,iSP15n,isite) = bio0_glob(:,iSPn,isite) * 0.00366d0 
!bio0_glob(:,iTR15n,isite) = bio0_glob(:,iTRn,isite) * 0.00366d0 
!bio0_glob(:,iDDA15n,isite) = bio0_glob(:,iDDAn,isite) * 0.00366d0 
!bio0_glob(:,iUN15n,isite) = bio0_glob(:,iUNn,isite) * 0.00366d0
!bio0_glob(:,iBA15n,isite) = bio0_glob(:,iBAn,isite) * 0.00366d0 
!bio0_glob(:,iPRT15n,isite) = bio0_glob(:,iPRTn,isite) * 0.00366d0 
!bio0_glob(:,iMZ15n,isite) = bio0_glob(:,iMZn,isite) * 0.00366d0 
!bio0_glob(:,iLDOM15n,isite) = bio0_glob(:,iLDOMn,isite) * 0.00366d0 
!bio0_glob(:,iSDOM15n,isite) = bio0_glob(:,iSDOMn,isite) * 0.00366d0 
!bio0_glob(:,iDET15n,isite) = bio0_glob(:,iDETn,isite) * 0.00366d0 
!bio0_glob(:,i15NH4,isite) = bio0_glob(:,iNH4,isite) * 0.00366d0 
!bio0_glob(:,i15NO3,isite) = bio0_glob(:,iNO3,isite) * 0.00366d0 
!bbcnsv_glob(iSP15n:iDET15n,:,isite) = 0.0d0 
!bbcnsv_glob(i15NH4,:,isite) = bbcnsv_glob(iNH4,:,isite) * 0.00366d0 
!bbcnsv_glob(i15NO3,:,isite) = bbcnsv_glob(iNO3,:,isite) * 0.00366d0 
!==========================================================
    end do siteloop
...
! 预留的 15N 代价验证索引：请在 cost.F90 中添加对应定义后取消注释 
! use cost, only : xc15nit, xcdon15n, xcpo15n, xcst15n
...
use eco_params, only: iPO4,iBAc,iprBAc,iSDOMc,iSDOMn,iSDOMp,&
                    iSPc,iTRc,iDDAc,iUNc,iPRTc,iDETc, &
                     ...
                     iUNn,iUNp,iPRTn,iPRTp, &
				     iDETn,iDETp, iSPchl, iTRchl,iDDAchl,iUNchl !Luo_MB
! 补充引入 15N 状态变量，用于代价函数计算
use eco_params, only: iSP15n, iTR15n, iDDA15n, iUN15n, iPRT15n, &
                  iDET15n, i15NO3, iSDOM15n
...
!=================================================================== 
! 以下为 15N 预留代价函数预测代码（包含硝酸盐、DON、PON）。 
! 使用前请务必在 cost.F90 的声明中加入 xc15nit, xcdon15n, xcpo15n 等变量 
!============================================================
! do idat = 1, ndat(xc15nit) 
! cpred(xc15nit, idat) = & 
! bio(z_cdat(xc15nit,idat), i15NO3, time_cdat(xc15nit,idat)) 
! enddo 

! do idat = 1, ndat(xcdon15n) 
! cpred(xcdon15n, idat) = & 
! bio(z_cdat(xcdon15n,idat), iSDOM15n, time_cdat(xcdon15n,idat))
! enddo 

! do idat = 1, ndat(xcpo15n) 
! cpred(xcpo15n, idat) = & 
! bio(z_cdat(xcpo15n,idat), iSP15n, time_cdat(xcpo15n,idat)) & 
! +bio(z_cdat(xcpo15n,idat), iTR15n, time_cdat(xcpo15n,idat)) & 
! +bio(z_cdat(xcpo15n,idat), iDDA15n, time_cdat(xcpo15n,idat)) & 
! +bio(z_cdat(xcpo15n,idat), iUN15n, time_cdat(xcpo15n,idat)) & 
! +bio(z_cdat(xcpo15n,idat), iPRT15n, time_cdat(xcpo15n,idat)) & 
! +bio(z_cdat(xcpo15n,idat), iDET15n, time_cdat(xcpo15n,idat)) 
! enddo
...
!=================================================================== 
! 以下为 15N 预留沉积物捕获器(Sediment trap)代价函数计算代码。
! 使用前请务必在 cost.F90 中添加对应定义 xcst15n 后取消注释。 
! 注：此处 15.0d0 代表 15N 的原子量，用以进行分子质量缩放。 
!================================================================== 
! nsed_pred = 0 
! do it=1,nt 
! it_temp = it 
! do ised=1,ndat_sed(xcst15n) 
! ised_loc = nz+1 
! if (z_csed(xcst15n,ised).lt.zifc(nz+1)) then 
! do while (z_csed(xcst15n,ised).lt.zifc(ised_loc))
! ised_loc = ised_loc - 1 
! end do
! endif 
! z2trap = z_csed(xcst15n,ised)-zifc(ised_loc) 
! t2trap = cdays(it)+(z2trap)/wnsv(iDET15n) 
! if ((t2trap.gt. btime_csed(xcst15n,ised)).and. & 
! (t2trap.le. etime_csed(xcst15n,ised))) then 
! csed_pred(xcst15n,ised) = csed_pred(xcst15n,ised)+wnsv(iDET15n) & 
! 15.0d0 * bio(ised_loc-1,iDET15n,it_temp)* & 
! exp(-z2trap*bioparams(iremin)*bioparams(iremin_prf_n)/wnsv(iDET15n)) 
! nsed_pred(ised) = nsed_pred(ised) + 1 
! endif 
! enddo 
! enddo 
! where (nsed_pred(1:ndat_sed(xcst15n)) > 0) 
! csed_pred(xcst15n,1:ndat_sed(xcst15n)) = & 
! csed_pred(xcst15n,1:ndat_sed(xcst15n))/nsed_pred(1:ndat_sed(xcst15n)) 
! end where
