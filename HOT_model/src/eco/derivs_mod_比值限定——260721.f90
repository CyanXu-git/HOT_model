...
!     Yangchun Xu 2026/04/29
! CVS: $Name: $
! 底物利用率最大为0.9,引入浓度保护值-20260518
!----------------------------------------------------------------------------
...
double precision :: growSP15nh4, growSP15no3, growSP15n,&
      ...
      u_nh4_SP, beta_nh4_SP, factor_nh4_SP, & 
      u_no3_SP, beta_no3_SP, factor_no3_SP, & 
      u_nh4_TR, beta_nh4_TR, factor_nh4_TR, & 
      u_no3_TR, beta_no3_TR, factor_no3_TR, & 
      u_nh4_DDA, beta_nh4_DDA, factor_nh4_DDA, & 
      u_no3_DDA, beta_no3_DDA, factor_no3_DDA, &
      u_nh4_UN, beta_nh4_UN, factor_nh4_UN, & 
      u_no3_UN, beta_no3_UN, factor_no3_UN, &  
      u_nh4_BA, beta_nh4_BA, factor_nh4_BA, & 
      u_no3_BA, beta_no3_BA, factor_no3_BA, &
      ...
      u_prt_graz, beta_prt_graz, factor_prt_graz, beta_nf, &
      u_tr_excr, beta_tr_excr,factor_tr_excr,&
      u_un_excr, beta_un_excr,factor_un_excr,&
      u_dda_excr, beta_dda_excr,factor_dda_excr, &
      u_BA_remi, beta_BA_remi, factor_BA_remi, &
      u_PRT_remi, beta_PRT_remi, factor_PRT_remi, &
      u_MZ_remi, beta_MZ_remi, factor_MZ_remi, &
      u_det_remi, beta_det_remi, factor_det_remi, &
      u_nitrf, beta_nitrf, factor_nitrf, smooth_factor
...
double precision :: fSP, fTR, fDDA, fUN, fBA, fPRT, fMZ, &
        fDET, fLDOM, fSDOM, fNH4, fNO3
...
! r_SDOMrefr    = bioparams(ir_SDOMrefr     )
  ...
  !----------------------------------------------
    ...
    ! SP
    if (y(iSPn) > eps .and. y(iSP15n) > 0.0d0) then
        fSP = y(iSP15n) / max(y(iSPn), eps)
        fSP = max(0.0d0, min(0.005d0, fSP))
    else
        fSP = 0.0d0
    endif
    rSP = fSP / max(1.0d0 - fSP, eps)
    
    ! TR
    if (y(iTRn) > eps .and. y(iTR15n) > 0.0d0) then
        fTR = y(iTR15n) / max(y(iTRn), eps)
        fTR = max(0.0d0, min(0.005d0, fTR))
    else
        fTR = 0.0d0
    endif
    rTR = fTR / max(1.0d0 - fTR, eps)
    
    ! DDA
    if (y(iDDAn) > eps .and. y(iDDA15n) > 0.0d0) then
        fDDA = y(iDDA15n) / max(y(iDDAn), eps)
        fDDA = max(0.0d0, min(0.005d0, fDDA))
    else
        fDDA = 0.0d0
    endif
    rDDA = fDDA / max(1.0d0 - fDDA, eps)
    
    ! UN
    if (y(iUNn) > eps .and. y(iUN15n) > 0.0d0) then
        fUN = y(iUN15n) / max(y(iUNn), eps)
        fUN = max(0.0d0, min(0.005d0, fUN))
    else
        fUN = 0.0d0
    endif
    rUN = fUN / max(1.0d0 - fUN, eps)
    
    ! BA
    if (y(iBAn) > eps .and. y(iBA15n) > 0.0d0) then
        fBA = y(iBA15n) / max(y(iBAn), eps)
        fBA = max(0.0d0, min(0.005d0, fBA))
    else
        fBA = 0.0d0
    endif
    rBA = fBA / max(1.0d0 - fBA, eps)
    
    ! PRT
    if (y(iPRTn) > eps .and. y(iPRT15n) > 0.0d0) then
        fPRT = y(iPRT15n) / max(y(iPRTn), eps)
        fPRT = max(0.0d0, min(0.005d0, fPRT))
    else
        fPRT = 0.0d0
    endif
    rPRT = fPRT / max(1.0d0 - fPRT, eps)
    
    ! MZ
    if (y(iMZn) > eps .and. y(iMZ15n) > 0.0d0) then
        fMZ = y(iMZ15n) / max(y(iMZn), eps)
        fMZ = max(0.0d0, min(0.005d0, fMZ))
    else
        fMZ = 0.0d0
    endif
    rMZ = fMZ / max(1.0d0 - fMZ, eps)
    
    ! DET
    if (y(iDETn) > eps .and. y(iDET15n) > 0.0d0) then
        fDET = y(iDET15n) / max(y(iDETn), eps)
        fDET = max(0.0d0, min(0.005d0, fDET))
    else
        fDET = 0.0d0
    endif
    rDET = fDET / max(1.0d0 - fDET, eps)
    
    ! LDOM
    if (y(iLDOMn) > eps .and. y(iLDOM15n) > 0.0d0) then
        fLDOM = y(iLDOM15n) / max(y(iLDOMn), eps)
        fLDOM = max(0.0d0, min(0.005d0, fLDOM))
    else
        fLDOM = 0.0d0
    endif
    rLDOM = fLDOM / max(1.0d0 - fLDOM, eps)
    
    ! SDOM
    if (y(iSDOMn) > eps .and. y(iSDOM15n) > 0.0d0) then
        fSDOM = y(iSDOM15n) / max(y(iSDOMn), eps)
        fSDOM = max(0.0d0, min(0.005d0, fSDOM))
    else
        fSDOM = 0.0d0
    endif
    rSDOM = fSDOM / max(1.0d0 - fSDOM, eps)
    
    ! NH4
    if (y(iNH4) > eps .and. y(i15NH4) > 0.0d0) then
        fNH4 = y(i15NH4) / max(y(iNH4), eps)
        fNH4 = max(0.0d0, min(0.005d0, fNH4))
    else
        fNH4 = 0.0d0
    endif
    rNH4 = fNH4 / max(1.0d0 - fNH4, eps)
    
    ! NO3
    if (y(iNO3) > eps .and. y(i15NO3) > 0.0d0) then
        fNO3 = y(i15NO3) / max(y(iNO3), eps)
        fNO3 = max(0.0d0, min(0.005d0, fNO3))
    else
        fNO3 = 0.0d0
    endif
    rNO3 = fNO3 / max(1.0d0 - fNO3, eps)
    ...
    !=======================================================================
      ...
      no3_limit= 0.01d0   !设为 0.01，低于此浓度强制关闭分馏
      nh4_limit= 0.001d0  !设为 0.001，低于此浓度强制关闭分馏

    !--------------------------------------------------------------------
    ! SP NH4 assimilation
    !--------------------------------------------------------------------
    u_nh4_SP = (growSPnh4* dt) / (y(iNH4)+eps) 
    u_nh4_SP = max(0.0d0, min(0.9d0, u_nh4_SP)) ! 数值约束在 [0, 1]
    
    if (y(iNH4) < nh4_limit) then
       factor_nh4_SP = 0.0d0  !情况B：底物浓度极低，饥饿状态，完全无分馏
    else if (u_nh4_SP < 1.0d-8) then 
       factor_nh4_SP = 1.0d0  !情况A：底物充足但利用率极低，执行最大强度分馏（了解析极限）
    else 
       factor_nh4_SP = -((1.0d0 - u_nh4_SP) / u_nh4_SP * log(1.0d0 - u_nh4_SP)) ! 正常 Rayleigh 过程
    end if
    ...
    beta_nh4_SP = rNH4 * &
        (1.0d0 - (epsilon_anh4/1000.0d0) * factor_nh4_SP)
    beta_nh4_SP = max(0.0d0, beta_nh4_SP)
    
    frac15_tmp = beta_nh4_SP / (1.0d0 + beta_nh4_SP)
    frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
    
    if (growSPnh4 > 0.0d0) then
        growSP15nh4 = growSPnh4 * frac15_tmp
        growSP15nh4 = max(0.0d0, min(growSP15nh4, growSPnh4))
    else
        growSP15nh4 = 0.0d0
    endif
    !--------------------------------------------------------------------
    ...
    u_no3_SP = max(0.0d0, min (0.9d0, u_no3_SP)) ! 数值约束在 [0, 1]
    
    if (y(iNO3) < no3_limit) then
       factor_no3_SP = 0.0d0  !情况B
    else if (u_no3_SP < 1.0d-8) then 
       factor_no3_SP = 1.0d0  !情况A
    else 
    ...
    beta_no3_SP = rNO3 * &
        (1.0d0 - (epsilon_ano3/1000.0d0) * factor_no3_SP)
    beta_no3_SP = max(0.0d0, beta_no3_SP)
    
    frac15_tmp = beta_no3_SP / (1.0d0 + beta_no3_SP)
    frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
    
    if (growSPno3 > 0.0d0) then
        growSP15no3 = growSPno3 * frac15_tmp
        growSP15no3 = max(0.0d0, min(growSP15no3, growSPno3))
    else
        growSP15no3 = 0.0d0
    endif    
    ...
    excrSP_1_15n = excrSP_1n * fSP ! 15N 排泄 1
    ...
    excrSP_2_15n = excrSP_2n *fSP ! 15N 排泄 2

    ! Aggregation of SP
        pomSPc   = r_pomSP * y(iSPc)*y(iSPc)
        pomSPn   = pomSPc * y(iSPn) / y(iSPc)
        pomSP15n = pomSPn * fSP      ! 15N 碎屑产生
    ...
    grazSPc = mu_PRT * y(iPRTc) * y(iSPc) * y(iSPc) &
     ...
     u_sp_graz = max(0.0d0, min(0.9d0, u_sp_graz)) ! 数值约束在 [0, 1]

     if (u_sp_graz < 1.0d-8) then 
     ...
     ! grazSP15n = grazSPn * (beta_sp_graz / (1.0d0 + beta_sp_graz))
      ...
      beta_sp_graz = rSP * &
          (1.0d0 - (epsilon_g/1000.0d0) * factor_sp_graz)
      beta_sp_graz = max(0.0d0, beta_sp_graz)
      
      frac15_tmp = beta_sp_graz / (1.0d0 + beta_sp_graz)
      frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
       
      if (grazSPn > 0.0d0) then
           grazSP15n = grazSPn * frac15_tmp
           grazSP15n = max(0.0d0, min(grazSP15n, grazSPn))
      else
           grazSP15n = 0.0d0
      endif 
   ...
   ! --------------------------------------------------------------------
    ...
    u_nh4_TR = max(0.0d0, min(0.9d0, u_nh4_TR)) ! 数值约束在 [0, 1]
    
    if (y(iNH4) < nh4_limit) then
       factor_nh4_TR = 0.0d0 
    else if (u_nh4_TR < 1.0d-8) then 
       factor_nh4_TR = 1.0d0  
    else
    ...
    !growTR15nh4 = growTRnh4 * (beta_nh4_TR / (1.0d0 + beta_nh4_TR))
     ...
     beta_nh4_TR = rNH4 * &
         (1.0d0 - (epsilon_anh4/1000.0d0) * factor_nh4_TR)
     beta_nh4_TR = max(0.0d0, beta_nh4_TR)
     
     frac15_tmp = beta_nh4_TR / (1.0d0 + beta_nh4_TR)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (growTRnh4 > 0.0d0) then
         growTR15nh4 = growTRnh4 * frac15_tmp
         growTR15nh4 = max(0.0d0, min(growTR15nh4, growTRnh4))
     else
         growTR15nh4 = 0.0d0
     endif
    ...
    u_no3_TR = max(0.0d0, min (0.9d0, u_no3_TR)) ! 数值约束在 [0, 1]
    
    if (y(iNO3) < no3_limit) then
         factor_no3_TR = 0.0d0  ! 情况B
    else if (u_no3_TR < 1.0d-8) then 
         factor_no3_TR = 1.0d0  ! 情况A
    else 
    ...
    !growTR15no3 = growTRno3 * (beta_no3_TR / (1.0d0 + beta_no3_TR))
     ...
     beta_no3_TR = rNO3 * &
         (1.0d0 - (epsilon_ano3/1000.0d0) * factor_no3_TR)
     beta_no3_TR = max(0.0d0, beta_no3_TR)
     
     frac15_tmp = beta_no3_TR / (1.0d0 + beta_no3_TR)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (growTRno3 > 0.0d0) then
         growTR15no3 = growTRno3 * frac15_tmp
         growTR15no3 = max(0.0d0, min(growTR15no3, growTRno3))
     else
         growTR15no3 = 0.0d0
     endif
    ...
    u_tr_excr = 0.5* r_excrTR_n * min(c1, Nfunc_tr_n)
    u_tr_excr = max(0.0d0, min(0.9d0, u_tr_excr))
   
    if (u_tr_excr < 1.0d-8) then 
     ...
     beta_tr_excr = R_std * &
         (1.0d0 - (epsilon_exnh4/1000.0d0) * factor_tr_excr)
     beta_tr_excr = max(0.0d0, beta_tr_excr)
     
     frac15_tmp = beta_tr_excr / (1.0d0 + beta_tr_excr)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (excrTR_nh4 > 0.0d0) then
         excrTR_15nh4 = excrTR_nh4 * frac15_tmp
         excrTR_15nh4 = max(0.0d0, min(excrTR_15nh4, excrTR_nh4))
     else
         excrTR_15nh4 = 0.0d0
     endif
    
    excrTR_1_15n = r_excrTR_1 * y(iTRn) * fTR+excrTR_15nh4
    ...
    !--------------------------------------------------------------------
     ...
     u_tr_graz = max(0.0d0, min(0.9d0, u_tr_graz))! 数值约束在 [0, 1]
     
     if (u_tr_graz < 1.0d-8) then 
     ...
     beta_tr_graz = max(0.0d0, beta_tr_graz)
     
     frac15_tmp = beta_tr_graz / (1.0d0 + beta_tr_graz)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (grazTRn > 0.0d0) then
         grazTR15n = grazTRn * frac15_tmp
         grazTR15n = max(0.0d0, min(grazTR15n, grazTRn))
     else
         grazTR15n = 0.0d0
     endif 
    ...
    !--------------------------------------------------------------------
     ...
     if (growDDAnf > 0.0d0) then
         growDDA15nf = growDDAnf * frac15_tmp
         growDDA15nf = max(0.0d0, min(growDDA15nf, growDDAnf))
     else
         growDDA15nf = 0.0d0
     endif
    ...
    u_nh4_DDA = max(0.0d0, min(0.9d0, u_nh4_DDA)) ! 数值约束在 [0, 1]
    
    if (y(iNH4) < nh4_limit) then
       factor_nh4_DDA = 0.0d0  ! 情况B
    else if (u_nh4_DDA < 1.0d-8) then 
       factor_nh4_DDA = 1.0d0  ! 情况A
    else 
    ...
    !计算 dda 同化NH4的特定 beta 值 
     beta_nh4_DDA = rNH4 * &
         (1.0d0 - epsilon_anh4/1000.0d0 * factor_nh4_DDA)
     beta_nh4_DDA = max(0.0d0, beta_nh4_DDA)
     
     frac15_tmp = beta_nh4_DDA / (1.0d0 + beta_nh4_DDA)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (growDDAnh4 > 0.0d0) then
         growDDA15nh4 = growDDAnh4 * frac15_tmp
         growDDA15nh4 = max(0.0d0, min(growDDA15nh4, growDDAnh4))
     else
         growDDA15nh4 = 0.0d0
     endif
    ...
    u_no3_DDA = max(0.0d0, min (0.9d0, u_no3_DDA)) ! 数值约束在 [0, 1]
    
    if (y(iNO3) < no3_limit) then
         factor_no3_DDA = 0.0d0  ! 情况B
    else if (u_no3_DDA < 1.0d-8) then 
         factor_no3_DDA = 1.0d0  ! 情况A
    else 
    ...
    beta_no3_DDA = rNO3 * &
         (1.0d0 - epsilon_ano3/1000.0d0 * factor_no3_DDA)
    beta_no3_DDA = max(0.0d0, beta_no3_DDA)
     
     frac15_tmp = beta_no3_DDA / (1.0d0 + beta_no3_DDA)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (growDDAno3 > 0.0d0) then
         growDDA15no3 = growDDAno3 * frac15_tmp
         growDDA15no3 = max(0.0d0, min(growDDA15no3, growDDAno3))
     else
         growDDA15no3 = 0.0d0
     endif
    ...
    !-------------------------------------------------------------------
     ! DDA NH4 excretion
    !--------------------------------------------------------------------
    u_dda_excr = 0.5* r_excrDDA_n * min(c1, Nfunc_dda_n)
    u_dda_excr = max(0.0d0, min(0.9d0, u_dda_excr))

    if (u_dda_excr < 1.0d-8) then 
     ...
     beta_dda_excr = R_std * &
         (1.0d0 - epsilon_exnh4/1000.0d0 * factor_dda_excr)
     beta_dda_excr = max(0.0d0, beta_dda_excr)
     
     frac15_tmp = beta_dda_excr / (1.0d0 + beta_dda_excr)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (excrDDA_nh4 > 0.0d0) then
         excrDDA_15nh4 = excrDDA_nh4 * frac15_tmp
         excrDDA_15nh4 = max(0.0d0, min(excrDDA_15nh4, excrDDA_nh4))
     else
         excrDDA_15nh4 = 0.0d0
     endif

     excrDDA_1_15n = r_excrDDA_1 * y(iDDAn) * fDDA+ excrDDA_15nh4
   ...
   ! DDA grazed
    ...
    u_dda_graz = max(0.0d0, min(0.9d0, u_dda_graz)) ! 数值约束在 [0, 1]
    if (u_dda_graz < 1.0d-8) then 
    ...
    beta_dda_graz = rDDA * &
         (1.0d0 - epsilon_g/1000.0d0 * factor_dda_graz)
     beta_dda_graz = max(0.0d0, beta_dda_graz)
     
     frac15_tmp = beta_dda_graz / (1.0d0 + beta_dda_graz)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (grazDDAn > 0.0d0) then
         grazDDA15n = grazDDAn * frac15_tmp
         grazDDA15n = max(0.0d0, min(grazDDA15n, grazDDAn))
     else
         grazDDA15n = 0.0d0
     endif
    ...
    if (growUNnf > 0.0d0) then
         growUN15nf = growUNnf * frac15_tmp
         growUN15nf = max(0.0d0, min(growUN15nf, growUNnf))
     else
         growUN15nf = 0.0d0
     endif
    ...
    u_nh4_UN = max(0.0d0, min(0.9d0, u_nh4_UN)) ! 数值约束在 [0, 1]
    
    if (y(iNH4) < nh4_limit) then
       factor_nh4_UN = 0.0d0  ! 情况B
    else if (u_nh4_UN < 1.0d-8) then 
       factor_nh4_UN = 1.0d0  ! 情况A
    else 
    ...
    beta_nh4_UN = rNH4 * &
    (1.0d0 - epsilon_anh4/1000.0d0 * factor_nh4_UN)
    beta_nh4_UN = max(0.0d0, beta_nh4_UN)
    
    frac15_tmp = beta_nh4_UN / (1.0d0 + beta_nh4_UN)
    frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
    
    if (growUNnh4 > 0.0d0) then
        growUN15nh4 = growUNnh4 * frac15_tmp
        growUN15nh4 = max(0.0d0, min(growUN15nh4, growUNnh4))
    else
        growUN15nh4 = 0.0d0
    endif
    ...
    u_no3_UN = max(0.0d0, min (0.9d0, u_no3_UN)) ! 数值约束在 [0, 1]
    
    if (y(iNO3) <no3_limit) then
         factor_no3_UN = 0.0d0  ! 情况B
    else if (u_no3_UN < 1.0d-8) then 
         factor_no3_UN = 1.0d0  ! 情况A
    else 
    ...
    !计算 UN 同化NO3的特定 beta 值 
     beta_no3_UN = rNO3 * &
         (1.0d0 - epsilon_ano3/1000.0d0 * factor_no3_UN)
     beta_no3_UN = max(0.0d0, beta_no3_UN)
     
     frac15_tmp = beta_no3_UN / (1.0d0 + beta_no3_UN)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (growUNno3 > 0.0d0) then
         growUN15no3 = growUNno3 * frac15_tmp
         growUN15no3 = max(0.0d0, min(growUN15no3, growUNno3))
     else
         growUN15no3 = 0.0d0
     endif
    ...
    !-------------------------------------------------------------------
     ! UN NH4 excretion
    !--------------------------------------------------------------------
    u_un_excr =  0.5 * r_excrUN_n * min(c1, Nfunc_un_n)
    u_un_excr = max(0.0d0, min(0.9d0, u_un_excr))
    
    if (u_un_excr < 1.0d-8) then 
    ...
    beta_un_excr = R_std * &
        (1.0d0 - epsilon_exnh4/1000.0d0 * factor_un_excr)
    beta_un_excr = max(0.0d0, beta_un_excr)
    
    frac15_tmp = beta_un_excr / (1.0d0 + beta_un_excr)
    frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
    
    if (excrUN_nh4 > 0.0d0) then
        excrUN_15nh4 = excrUN_nh4 * frac15_tmp
        excrUN_15nh4 = max(0.0d0, min(excrUN_15nh4, excrUN_nh4))
    else
        excrUN_15nh4 = 0.0d0
    endif
    
    excrUN_1_15n = r_excrUN_1 * y(iUNn) * fUN +excrUN_15nh4
   ...
   ! UN grazed
    ...
    u_un_graz = max(0.0d0, min(0.9d0, u_un_graz)) ! 数值约束在 [0, 1]
    if (u_un_graz < 1.0d-8) then 
    ...
    !计算UN被捕食的特定 beta 值和UN被捕食15N吸收方程
     beta_un_graz = rUN * &
         (1.0d0 - epsilon_g/1000.0d0 * factor_un_graz)
     beta_un_graz = max(0.0d0, beta_un_graz)
     
     frac15_tmp = beta_un_graz / (1.0d0 + beta_un_graz)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (grazUNn > 0.0d0) then
         grazUN15n = grazUNn * frac15_tmp
         grazUN15n = max(0.0d0, min(grazUN15n, grazUNn))
     else
         grazUN15n = 0.0d0
     endif
    ...
    ! 15N DON 摄取
    growBA15ldon = growBAldon * fLDOM
    growBA15sdon = growBAsdon * fSDOM
    ...
    u_nh4_BA = max(0.0d0, min(0.9d0, u_nh4_BA)) ! 数值约束在 [0, 1]
    
    if (y(iNH4) < nh4_limit) then
       factor_nh4_BA = 0.0d0  ! 情况B
    else if (u_nh4_BA < 1.0d-8) then 
       factor_nh4_BA = 1.0d0  ! 情况A
    else 
    ...
    !计算BA 同化NH4的特定 beta 值 
     beta_nh4_BA = rNH4 * &
         (1.0d0 - epsilon_anh4/1000.0d0 * factor_nh4_BA)
     beta_nh4_BA = max(0.0d0, beta_nh4_BA)
     
     frac15_tmp = beta_nh4_BA / (1.0d0 + beta_nh4_BA)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (growBAnh4 > 0.0d0) then
         growBA15nh4 = growBAnh4 * frac15_tmp
         growBA15nh4 = max(0.0d0, min(growBA15nh4, growBAnh4))
     else
         growBA15nh4 = 0.0d0
     endif
    ...
    u_no3_BA = max(0.0d0, min (0.9d0, u_no3_BA)) ! 数值约束在 [0, 1]
    
    if (y(iNO3) < no3_limit) then
         factor_no3_BA = 0.0d0  ! 情况B
    else if (u_no3_BA < 1.0d-8) then 
         factor_no3_BA = 1.0d0  ! 情况A
    else 
    ...
    !计算 BA 同化NO3的特定 beta 值 
     beta_no3_BA = rNO3 * &
         (1.0d0 - epsilon_ano3/1000.0d0 * factor_no3_BA)
     beta_no3_BA = max(0.0d0, beta_no3_BA)
     
     frac15_tmp = beta_no3_BA / (1.0d0 + beta_no3_BA)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (growBAno3 > 0.0d0) then
         growBA15no3 = growBAno3 * frac15_tmp
         growBA15no3 = max(0.0d0, min(growBA15no3, growBAno3))
     else
         growBA15no3 = 0.0d0
     endif
    ...
    ELSE !Phosphorus in short
         ...
         remiBAp = c0
   END IF
   excrBA15n= excrBAn* fBA

  ! 计算BA再矿化的特定 beta 值和 BA再矿化的15N吸收方程
    beta_BA_remi = rBA * &
         (1.0d0 - epsilon_remin/1000.0d0)
     beta_BA_remi = max(0.0d0, beta_BA_remi)
     
     frac15_tmp = beta_BA_remi / (1.0d0 + beta_BA_remi)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (remiBAn > 0.0d0) then
         remiBA15n = remiBAn * frac15_tmp
         remiBA15n = max(0.0d0, min(remiBA15n, remiBAn))
     else
         remiBA15n = 0.0d0
     endif
    ...
    u_ba_graz = max(0.0d0, min(0.9d0, u_ba_graz)) ! 数值约束在 [0, 1]
    if (u_ba_graz < 1.0d-8) then 
    ...
    !  计算BA被捕食的特定 beta 值和BA被捕食15N吸收方程 
     beta_ba_graz = rBA * &
         (1.0d0 - epsilon_g/1000.0d0 * factor_ba_graz)
     beta_ba_graz = max(0.0d0, beta_ba_graz)
     
     frac15_tmp = beta_ba_graz / (1.0d0 + beta_ba_graz)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (grazBAn > 0.0d0) then
         grazBA15n = grazBAn * frac15_tmp
         grazBA15n = max(0.0d0, min(grazBA15n, grazBAn))
     else
         grazBA15n = 0.0d0
     endif
    ...
    excrPRTsdom15n = excrPRTsdomn * fPRT
    ...
    excrPRTsdom2_15n = excrPRTsdom2n * fPRT
    ...
    !计算PRT再矿化的特定 beta 值和PRT再矿化的15N吸收方程 
     beta_PRT_remi = rPRT * &
         (1.0d0 - epsilon_remin/1000.0d0)
     beta_PRT_remi = max(0.0d0, beta_PRT_remi)
     
     frac15_tmp = beta_PRT_remi / (1.0d0 + beta_PRT_remi)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (remiPRTn > 0.0d0) then
         remiPRT15n = remiPRTn * frac15_tmp
         remiPRT15n = max(0.0d0, min(remiPRT15n, remiPRTn))
     else
         remiPRT15n = 0.0d0
     endif
    ...
    grazPRTc = mu_MZ * y(iMZc) * y(iPRTc) * y(iPRTc) &
     ...
     u_prt_graz = max(0.0d0, min(0.9d0, u_prt_graz)) ! 数值约束在 [0, 1]
     if (u_prt_graz < 1.0d-8) then 
     factor_prt_graz = 1.0d0 
     else
        factor_prt_graz= -((1.0d0 - u_prt_graz) / u_prt_graz* log(1.0d0 - u_prt_graz))
     end if 

     !  计算PRT被捕食的特定 beta 值和PRT被捕食15N吸收方程
     beta_prt_graz = rPRT * &
         (1.0d0 - epsilon_g/1000.0d0 * factor_prt_graz)
     beta_prt_graz = max(0.0d0, beta_prt_graz)
     
     frac15_tmp = beta_prt_graz / (1.0d0 + beta_prt_graz)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (grazPRTn > 0.0d0) then
         grazPRT15n = grazPRTn * frac15_tmp
         grazPRT15n = max(0.0d0, min(grazPRT15n, grazPRTn))
     else
         grazPRT15n = 0.0d0
     endif
    ...
    ! 4. adjust body stoichiometry by excreting semi-labile DOM
        ...
        excrMZsdom2_15n = excrMZsdom2n *  fMZ
    ...
    beta_MZ_remi = rMZ * &
         (1.0d0 - epsilon_remin/1000.0d0)
     beta_MZ_remi = max(0.0d0, beta_MZ_remi)
     
     frac15_tmp = beta_MZ_remi / (1.0d0 + beta_MZ_remi)
     frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
     if (remiMZn > 0.0d0) then
         remiMZ15n = remiMZn * frac15_tmp
         remiMZ15n = max(0.0d0, min(remiMZ15n, remiMZn))
     else
         remiMZ15n = 0.0d0
     endif
    ...
    !-----------------------------------------------------------------------
       ...
       beta_det_remi = rDET * &
         (1.0d0 - epsilon_remin/1000.0d0)
        beta_det_remi = max(0.0d0, beta_det_remi)
     
       frac15_tmp = beta_det_remi / (1.0d0 + beta_det_remi)
       frac15_tmp = max(0.0d0, min(0.005d0, frac15_tmp))
     
       if (disDETn > 0.0d0) then
         disDET15n = disDETn * frac15_tmp
         disDET15n = max(0.0d0, min(disDET15n, disDETn))
       else
         disDET15n = 0.0d0
       endif
    ...
    !--------------------------------------------------------------------
     ...
     u_nitrf = max(0.0d0, min(0.9d0, u_nitrf))
     
     if (y(iNH4) < nh4_limit) then
         factor_nitrf = 0.0d0
     else if (u_nitrf < 1.0d-8) then
         factor_nitrf = 1.0d0
     else
         factor_nitrf = -((1.0d0-u_nitrf)/u_nitrf * &
                           log(1.0d0-u_nitrf))
     endif
     
     beta_nitrf = rNH4 * &
         (1.0d0 - epsilon_nitrf/1000.0d0 * factor_nitrf)
     beta_nitrf = max(0.0d0, beta_nitrf)
     ...
     if (nitrf > 0.0d0) then
         ...
         nitrf15 = max(0.0d0, min(nitrf15, nitrf))
     else
         nitrf15 = 0.0d0
     endif
    ...
    dydtt_diag(igrowDDA15n)      = growDDA15n/SecPerDay
