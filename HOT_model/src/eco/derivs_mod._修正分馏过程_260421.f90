...
!    Yangchun Xu 2026/04/21
!    CVS: $Name: $
!    修正15N过程的分馏公式，利用率调整为0.95-2026/04/21
!----------------------------------------------------------------------------
...
double precision, parameter :: f15n_atm = 0.0036765d0
...
! r_SDOMrefr    = bioparams(ir_SDOMrefr     )
  ...
  !----------------------------------------------
   ...
   ! 添加15N同位素分馏效应
    ...
    u_nh4 = max(0.0d0, min(0.8d0, u_nh4)) ! 数值约束在 [0, 1]
    ...
    growBAno3
    u_no3 = (total_no3_util* dt) /(y(iNO3) +eps)
    u_no3 = max(0.0d0, min (0.8d0, u_no3)) ! 数值约束在 [0, 1]
    ...
    grazSPc = mu_PRT * y(iPRTc) * y(iSPc) * y(iSPc) &
     ...
     u_sp_graz = max(0.0d0, min(0.8d0, u_sp_graz)) ! 数值约束在 [0, 1]
   ...
   END IF  
    ...
    u_tr_excr = max(0.0d0, min(0.8d0, u_tr_excr))
    ...
    u_tr_graz = max(0.0d0, min(0.8d0, u_tr_graz))! 数值约束在 [0, 1]
...
growDDA15no3 = growDDAno3 * (beta_no3 / (1.0d0 + beta_no3)) 
    ...
    u_dda_excr = max(0.0d0, min(0.8d0, u_dda_excr))
...
pomDDA15n = pomDDAn *(rDDA/(1.0d0+rDDA))
   ...
   ! DDA grazed
    ...
    u_dda_graz = max(0.0d0, min(0.8d0, u_dda_graz)) ! 数值约束在 [0, 1]
   ...
   ! 15N 吸收与固氮
    ...
    !--------------------------------------------------------------------
     ! UN NO3 Assimilation 
    !--------------------------------------------------------------------
    growUN15no3 = growUN15no3 * (beta_no3/ (1.0d0 + beta_no3))
    ...
    u_un_excr = max(0.0d0, min(0.8d0, u_un_excr))
...
! UN grazed
    ...
    u_un_graz = max(0.0d0, min(0.8d0, u_un_graz)) ! 数值约束在 [0, 1]
...
!--------------------------------------------------------------------
    ...
    growBA15n = growBA15nh4 + growBA15no3 
...
!  计算BA再矿化的特定 beta 值 
   u_BA_remi= (remiBAn* dt)/ (y(iBAn) +eps)
   u_BA_remi = max(0.0d0, min(0.8d0, u_BA_remi)) ! 数值约束在 [0, 1]
   if (u_BA_remi < 1.0d-8) then 
      factor_BA_remi = 1.0d0 
   else
     factor_BA_remi = -((1.0d0 - u_BA_remi) / u_BA_remi * log(1.0d0 - u_BA_remi))
  end if 
       beta_BA_remi = rBA * (1.0d0 - (epsilon_remin/1000.0d0) * factor_BA_remi)
   ...
   remiBA15n = remiBAn * (beta_BA_remi/ (1.0d0 + beta_BA_remi))
    ...
    u_ba_graz = max(0.0d0, min(0.8d0, u_ba_graz)) ! 数值约束在 [0, 1]
    ...
    remiPRTn = MAX(r_PRTremi*(y(iPRTn)-q_PRT_n * y(iPRTc)), &
     ...
     !  计算PRT再矿化的特定 beta 值 
     !  计算PRT再矿化的特定 beta 值 
     u_PRT_remi= (remiPRTn* dt)/ (y(iPRTn) +eps)
     u_PRT_remi = max(0.0d0, min(0.8d0, u_PRT_remi)) ! 数值约束在 [0, 1]
     if (u_PRT_remi < 1.0d-8) then 
         factor_PRT_remi = 1.0d0 
     else
         factor_PRT_remi = -((1.0d0 - u_PRT_remi) / u_PRT_remi * log(1.0d0 - u_PRT_remi))
     end if 
         beta_PRT_remi = rPRT * (1.0d0 - (epsilon_remin/1000.0d0) * factor_PRT_remi)
     !  PRT再矿化的15N吸收方程
    ...
    grazPRTc = mu_MZ * y(iMZc) * y(iPRTc) * y(iPRTc) &
     ...
     u_prt_graz = max(0.0d0, min(0.8d0, u_prt_graz)) ! 数值约束在 [0, 1]
    ...
    ! 4. adjust body stoichiometry by excreting semi-labile DOM
        ...
        excrMZsdom2_15n = excrMZsdom2n * (rMZ/(1+ rMZ))
   ...
   !  计算MZ再矿化的特定 beta 值 
     u_MZ_remi= (remiMZn* dt)/ (y(iMZn) +eps)
     u_MZ_remi = max(0.0d0, min(0.8d0, u_MZ_remi)) ! 数值约束在 [0, 1]
    if (u_MZ_remi < 1.0d-8) then 
       factor_MZ_remi = 1.0d0 
   else
   factor_MZ_remi = -((1.0d0 - u_MZ_remi) / u_MZ_remi * log(1.0d0 - u_MZ_remi))
   end if 
   beta_MZ_remi = rMZ * (1.0d0 - (epsilon_remin/1000.0d0) * factor_MZ_remi)
   ...
   remiMZ15n = remiMZn * (beta_MZ_remi/ (1.0d0 + beta_MZ_remi))
    ...
    !-----------------------------------------------------------------------
       ...
       !  计算DET再矿化的特定 beta 值 
       u_det_remi= (disDETn * dt)/ (y(iDETn) +eps)
       u_det_remi = max(0.0d0, min(0.8d0, u_det_remi)) ! 数值约束在 [0, 1]
       if (u_det_remi < 1.0d-8) then 
           factor_det_remi = 1.0d0 
       else
          factor_det_remi = -((1.0d0 - u_det_remi) / u_det_remi * log(1.0d0 - u_det_remi))
       end if 
       beta_det_remi = rDET * (1.0d0 - (epsilon_remin/1000.0d0) * factor_det_remi)
       ! DET再矿化的15N吸收方程
    ...
    !-----------------------------------------------------------------------
     ...
     u_nitrf = max(0.0d0, min(0.8d0, u_nitrf)) ! 数值约束在 [0, 1]
