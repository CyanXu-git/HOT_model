...
! r_SDOMrefr    = bioparams(ir_SDOMrefr     )
  ...
  !----------------------------------------------
    ...
    !-----------------------------------------------------------------------
    ! 对3种固氮藻的15N/14N 比值进行低浓度约束
    !-----------------------------------------------------------------------
    if (y(iTRn) > eps .and.   &
    	y(iTR15n) >= 0.0d0 .and.  &
    	y(iTR15n) < y(iTRn)) then
   ...
   if (y(iUNn) > eps .and.   &
    ...
    rSDOM = y(iSDOM15n) / (y(iSDOMn)- y(iSDOM15n) + eps)
    ...
    excrSP_1_15n = excrSP_1n * (rSP/(1.0d0+ rSP)) ! 15N 排泄 1
    ...
    excrSP_2_15n = excrSP_2n * (rSP/(1.0d0+ rSP))! 15N 排泄 2

    ! Aggregation of SP
        pomSPc   = r_pomSP * y(iSPc)*y(iSPc)
        pomSPn   = pomSPc * y(iSPn) / y(iSPc)
        pomSP15n = pomSPn *(rSP/(1.0d0+rSP))       ! 15N 碎屑产生
   ...
   ! --------------------------------------------------------------------
    ...
    if (y(iNH4) < nh4_limit) then
       factor_nh4_TR = 0.0d0  ! 情况B
    else if (u_nh4_TR < 1.0d-8) then 
       factor_nh4_TR = 1.0d0  ! 情况A
    else 
    ...
    excrTR_1_15n = r_excrTR_1 * y(iTRn) * (rTR/(1.0d0+rTR))+excrTR_15nh4
    ...
    excrDDA_1_15n = r_excrDDA_1 * y(iDDAn) * (rDDA/(1.0d0+rDDA))+ excrDDA_15nh4
   ...
   ! DDA grazed
    ...
    !        - pomDDAn)/ SecPerDay
    !dydtt(iDDA15n) = (growDDA15n - excrDDA_1_15n - excrDDA_2_15n&
    !        - grazDDA15n - pomDDA15n)/ SecPerDay
   ...
   ! 15N 吸收与固氮
    ...
    excrUN_1_15n = r_excrUN_1 * y(iUNn) * (rUN/(1.0d0+rUN))+excrUN_15nh4
   ...
   ! UN grazed
    ...
    growBA15sdon = growBAsdon * (rSDOM/(1.0d0+rSDOM))
  ...
  ! BA再矿化的15N吸收方程
   remiBA15n = remiBAn * (beta_BA_remi/ (1.0d0 + beta_BA_remi))
    ...
    !remiMZ15n = remiMZn * (rMZ/(1.0d0+rMZ))
    ...
    u_nitrf = max(0.0d0, min(0.90d0, u_nitrf)) ! 数值约束在 [0, 1]

    if (y(iNH4) < nh4_limit) then
        factor_nitrf = 0.0d0  ! 情况B
    else if (u_nitrf < 1.0d-8) then
        factor_nitrf = 1.0d0  ! 情况A
    else
